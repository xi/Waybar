...
std::string maybe_str(const char *s) {
...
static void keyboard_modifiers(void *data, struct wl_keyboard *wl_keyboard, uint32_t serial,
  ...
  static_cast<KeyboardState *>(data)->handle_modifiers(mods_depressed, mods_latched, mods_locked, group);
}
...
KeyboardState::KeyboardState(const std::string &id, const waybar::Bar &bar, const Json::Value &config)
    : waybar::AModule(config, "keyboard-state", id, "{}"),
      ...
      scrolllock_format_(config_["format"].isString() ? config_["format"].asString()
                         : config_["format"]["scrolllock"].isString()
                             ...
                             : "{name} {icon}"),
      tooltip_format_(config_["tooltip-format"].isString()
                      ? config_["tooltip-format"].asString() : ""),
      ...
      icon_unlocked_(config_["format-icons"]["unlocked"].isString()
                     ...
                     : "unlocked"),
      hide_single_(config["hide-single-layout"].isBool()
                   && config["hide-single-layout"].asBool()),
      seat_{nullptr},
      keyboard_{nullptr},
...
void KeyboardState::update_layout(std::string full_name) {
  ...
  auto display_layout = trim(fmt::format(
    fmt::runtime(layout_format_),
    fmt::arg("short", short_name),
    fmt::arg("shortDescription", short_description),
    fmt::arg("long", full_name),
    fmt::arg("variant", variant),
    fmt::arg("flag", country_flag(short_name))));
  layout_label_.set_markup(display_layout);

  if (tooltipEnabled()) {
    if (tooltip_format_ != "") {
      auto tooltip_display_layout = trim(
          fmt::format(fmt::runtime(tooltip_format_),
                      fmt::arg("short", short_name),
                      fmt::arg("shortDescription", short_description),
                      fmt::arg("long", full_name),
                      fmt::arg("variant", variant),
...
void KeyboardState::update_led(Gtk::Label *label, std::string format, std::string name, bool locked) {
  std::string text = fmt::format(fmt::runtime(format),
                                 fmt::arg("icon", locked ? icon_locked_ : icon_unlocked_),
                                 fmt::arg("name", name));
